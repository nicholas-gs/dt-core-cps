...
from lane_filter.include import LaneFilterInterface
...
try:
    from lane_filter.include.visualization import plot_phi_d_diagram_bgr
except ImportError:
    pass
...
class LaneFilterHistogram(LaneFilterInterface):
    ...
    Lane pose is defined as the tuple (`d`, `phi`) : lateral deviation and
    angulare deviation from the center of the lane.
    Predict step : Uses the estimated linear and angular velocities to predict
        the change in the lane pose.
    Update Step : The filter receives a segment list. For each segment,
        it extracts the corresponding lane pose "votes", and adds it to the
        corresponding part of the histogram.
    Best estimate correspond to the slot of the histogram with the
        highest voted value.
    Args:
    ...
    def predict(self, dt, v, w):
        ...
        # there has got to be a better/cleaner way to do this -
        # just applying the process model to translate each cell value
    ...
    # get the distance from the center of the Duckiebot to the center
    # point of a segment
...
if all([module in sys.modules for module in
            ['duckietown_code_utils', 'lane_filter.include.visualization']]):
    def get_plot_phi_d(self, ground_truth=None) -> dtu.NPImageBGR:
        # if not all([module in sys.modules for module in
        #     ['duckietown_code_utils', 'lane_filter.include.visualization']]):
