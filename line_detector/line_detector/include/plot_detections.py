...
def plotSegments(image, detections, default_color=False):
    ...
    for color_range, det in list(detections.items()):
        ...
        for line in det.lines:
            im = cv2.line(im, (line[0], line[1]), (line[2], line[3]),
                color=(0, 0, 0), thickness=5)
            im = cv2.line(
                im, (line[0], line[1]), (line[2], line[3]),
                color=tuple([int(x) for x in color]), thickness=2
            )
    return im
...
def plotMaps(image, detections):
    """
    Draws a set of color filter maps (the part of the images falling in a given
    color range) on an image.
    Args:
        image (:obj:`numpy array`): an image
            detections (`dict`): a dictionary that has keys
            :py:class:`ColorRange` and values :py:class:`Detection`
    Returns:
        :obj:`numpy array`: the image with the line segments drawn on top of it.
    """
