...
class ColorRange:
    """The Color Range class holds one or multiple color ranges. It can easily
    be generated with the :py:meth:`fromDict` class method and extends the
    `OpenCV's inRange <https://docs.opencv.org/3.4/d2/de8/group__core__array.html#ga48af0ab51e36436c5d04340e036ce981>`_
    method to work with multiple color ranges.
All colours must be given in ``HSV`` space.
    Args:
        low (:obj:`numpy array`): An ``Nx3`` array with the low ends of
            ``N`` colour ranges.
    ...
    def fromDict(cls, dictionary, representation=None):
        ...
        If multi-entry ranges are provided (e.g. if you are interested in yellow
        and white), then each should have keys ``high_X`` and ``low_X``,
        see example bellow:
        Examples:
            Single-entry color range::
                { 'low': [0,0,150], 'high': [180,60,255] }
            Multi-entry color range::
                { 'low_1': [0,0,150], 'high_1': [180,60,255],
                'low_2': [165,140,100], 'high_2': [180,255,255] }
        Args:
    ...
    def inRange(self, image):
        ...
        In other words, returns a binary map with 1 for the pixels of the input
        image that fall in at least one of
        the color ranges.
        Args:
    ...
    def representative(self):
        """
        Provides an representative color for this color range. This is the
        average color of the first range (if more than one ranges are set).
        Returns:
